...
|------------|-----------------------------|------------------|
| delta-concept (`context-delta/prompts/delta-concept.md`) | `docs/concept.md` を `## Concept Summary` / `## CLI vs Prompt Responsibilities` / `## Localisation Strategy` / `## context-delta/ Directory Structure` で構成。責務表とディレクトリツリーを含む。 | markdownlint で見出し順と表構造を確認 |
